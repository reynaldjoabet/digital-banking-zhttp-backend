...
final case class EbankAppServer(
  ...
  /** Composes the routes together, returning a single HttpApp.
    */
  val allRoutes: HttpApp[Any, Throwable] = customerRoutes.routes ++ bankAccountRoutes.routes
  ...
  def start: ZIO[MigrationService with BankAccountService with CustomerService, Throwable, Unit] =
    for {
      ...
      _ <-
        ZIO.foreach(customers)(customer =>
          accountService.createCurrentAccount(
            balance = 9000,
            customerId = customer.customerId,
            overDraft = Some(800.0),
          )
        )
      _ <-
        ZIO.foreach(customers)(customer =>
      ...
      _ <-
        ZIO.foreach(accounts)(account =>
          accountService.credit(account.accountId, 10000 + amount, "Credit")
        )
      amount1 <- Random.nextIntBetween(1, 190)
      _ <-
        ZIO.foreach(accounts)(account =>
          accountService.debit(account.accountId, amount = amount1, "Debit")
        )
