...
/** CustomerRoutes is a service that provides the routes for the CustumerService API. The routes
  ...
  val routes: Http[Any, Throwable, Request, Response] = Http.collectZIO[Request] {
    ...
    case req @ Method.POST -> !! / "customers" =>
      for {
        ...
        createCustomer <- ZIO
          .fromEither(body.fromJson[CreateCustomer])
    ...
    /** Updates a single Customer found by their parsed ID using the information parsed from the
      * UpdateCustomer request and returns a 200 status code indicating success.
      */
      ...
      service.get(customerId.toInt).flatMap {
        ...
        case Some(customer) =>
          for {
            body <- req.bodyAsString.orElseFail(MissingBodyError)
            updateCustomer <- ZIO
              .fromEither(body.fromJson[UpdateCustomer])
              .mapError(_ => InvalidJsonBody)
            _ <- service.update(
              customer.customerId,
              updateCustomer.name,
...
object CustomerRoutes {
  ...
  val layer: ZLayer[CustomerService, Nothing, CustomerRoutes] = ZLayer.fromFunction(
    CustomerRoutes.apply _
  )
