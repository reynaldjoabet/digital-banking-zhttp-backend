...
/** BankAccountRoutes is a service that provides the routes for the CustumerService API. The routes
  ...
  */
final case class BankAccountRoutes(
...
//debit
    case req @ Method.POST -> !! / "accounts" / "debit" =>
      for {
        ...
        _ <- accountService.debit(
          debitAccount.accountId,
          debitAccount.amount,
          debitAccount.description,
        )
    ...
    case req @ Method.POST -> !! / "accounts" / "transfer" =>
      for {
        ...
        _ <- accountService.transfer(
          transferRequest.accountSource,
          transferRequest.accountDestination,
...
/** Here in the companion object we define the layer that will be used to provide the routes for the
  * BankAccountService API.
  */
object BankAccountRoutes {

  val layer: ZLayer[BankAccountService with AccountOperationService, Nothing, BankAccountRoutes] =
    ZLayer.fromFunction(BankAccountRoutes.apply _)
